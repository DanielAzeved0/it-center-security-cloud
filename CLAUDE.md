...
O backlog fica em `docs/development/TASKS.md` (fonte de verdade, decai rápido — confira antes de assumir que algo está pendente). Snapshot: EPICs 1–13 concluídas (fundação, backend, banco, agente, dashboard, SOC Light, deploy cloud, agente em produção, validação fim a fim, dashboard operacional, SOC Light pendências, governança/autenticação, operação/segurança de produção — restore, rollback e renovação TLS já validados de verdade em produção). EPIC 16 (hardening do agente Windows — ver ADR-025) e EPIC 18 (orquestração de agentes de IA usando recursos nativos do Claude Code, sem Strix/OpenAI/Gemini — ver ADR-026 e `docs/development/AI_WORKFLOW.md`) também concluídas, a primeira exceto assinatura de código (code-signing), pendente de certificado. EPIC 14 (Wazuh, OpenVAS, multiempresa, billing, SaaS) é futuro, fora do escopo. **Frentes ativas agora:** EPIC 15 (adoção de Terraform via import — módulos e ADR prontos, mas import dos recursos reais ainda não executado) e EPIC 17 (hardening do dashboard/frontend: migrar token de `localStorage` para cookie `httpOnly`, adicionar security headers e `middleware.ts`).
