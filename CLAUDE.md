...
O backlog fica em `docs/development/TASKS.md` (fonte de verdade, decai rápido — confira antes de assumir que algo está pendente). Snapshot: EPICs 1–13 concluídas (fundação, backend, banco, agente, dashboard, SOC Light, deploy cloud, agente em produção, validação fim a fim, dashboard operacional, SOC Light pendências, governança/autenticação, operação/segurança de produção — restore, rollback e renovação TLS já validados de verdade em produção). EPIC 14 (Wazuh, OpenVAS, multiempresa, billing, SaaS) é futuro, fora do escopo. **Frentes ativas agora:** EPIC 15 (adoção de Terraform via import — módulos e ADR prontos, mas import dos recursos reais ainda não executado) e EPIC 16 (hardening do agente Windows: API key em texto puro no `config.json`, cache offline sem quarentena para arquivo corrompido, sem rotação de logs/cache, medição de CPU imprecisa, scripts não assinados — ver ADR-025, que reafirma manter PowerShell em vez de reescrever).
