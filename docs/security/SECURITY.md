...
* Cache offline (`cache\checkin-*.json`, gravado por `Save-AgentOfflinePayload` quando o check-in falha) contém o payload completo do check-in — incluindo `local_admins`, status de firewall/Defender/RDP, IP/MAC/número de série e o inventário completo de programas instalados. O hardening da EPIC 16 aplicou ACL restritiva (SYSTEM/Administrators) somente em `config.json`; `logs\` e `cache\` herdam a ACL padrão de `Program Files` (leitura para `Users`), então qualquer usuário local não administrador consegue ler esses dados enquanto houver cache pendente. Risco conhecido; correção prevista na EPIC 28 (`docs/development/TASKS.md`): estender a mesma lógica de `Protect-AgentConfigFile` para `logs\` e `cache\`.
...
* **Path Traversal**: nenhum endpoint aceita caminho de arquivo como entrada do usuário para leitura em disco no backend. O proxy interno do dashboard (`frontend/dashboard/app/api/backend/[...path]/route.ts`) valida uma allowlist explícita de prefixos de rota, mas essa validação roda sobre o path ainda codificado — a URL final repassada ao backend só é montada depois, com `new URL()`, que normaliza `..` nesse momento. Um segmento como `api/v1/machines%2f..%2f..%2f..%2fdocs` passa na allowlist (começa com `api/v1/machines`) e resolve para `/docs` no backend, expondo o Swagger/OpenAPI da API interna sem autenticação — o backend não desativa `docs_url`/`redoc_url` em produção (`backend/app/main.py`). **Risco conhecido e pendente de correção (EPIC 28, `docs/development/TASKS.md`)**: até lá, o `[...path]` dinâmico do Next.js deve ser tratado como um encaminhador que pode ser contornado com paths codificados, não como uma allowlist estanque.
...
* `POST /api/v1/auth/login` não tem rate limit nem lockout de conta hoje — nem no Nginx (só existe `limit_req_zone` para a zona `agent_checkins`, `infra/nginx/nginx.conf.template`) nem na aplicação. Mitigação parcial já existente: mensagem de erro genérica (não confirma se o e-mail existe) e log de auditoria (`auth.login_failed`) a cada tentativa. Essa mensagem genérica, porém, não fecha completamente a enumeração de contas: o código faz curto-circuito (`user is None or status != "active" or not verify_password(...)`, `backend/app/routes/auth.py`) e só roda o PBKDF2 de 210.000 iterações — lento por natureza — quando o usuário existe e está ativo; a diferença de tempo de resposta entre um e-mail existente e um inexistente permite enumerar contas por timing attack mesmo com a mensagem idêntica. Correção prevista na EPIC 28 (`docs/development/TASKS.md`): rodar `verify_password` contra um hash dummy também quando o usuário não existir/estiver inativo, para equalizar o tempo de resposta. Uma correção efetiva de rate limit/lockout exigiria planejamento e ADR formal antes de implementação, conforme `docs/development/CONTRIBUTING.md` — não é tratada como pendência com prazo, e sim como risco aceito enquanto o MVP roda em escala pequena.
...
* Personificação de máquina via `AGENT_API_KEY` compartilhada: o backend valida apenas uma chave global (`X-Agent-Api-Key`) e identifica a máquina somente pelo `hostname` autorreportado no payload — não há vínculo servidor-side entre a chave usada e uma máquina específica. Quem extrai a chave de uma única máquina comprometida (`config.json` em texto puro, ADR-025) pode forjar check-in em nome de qualquer outro hostname já cadastrado/autorizado, sobrescrevendo IP/MAC/número de série e inventário de programas reais dessa outra máquina com dados forjados, sem disparar `unknown_asset` (que só reage a hostname fora da allowlist, não a uso indevido da chave). Risco conhecido e aceito enquanto não existir credencial por máquina — detalhado em `docs/security/AUTH.md` (seção "API Key por Agente") e rastreado na EPIC 28 (`docs/development/TASKS.md`).
