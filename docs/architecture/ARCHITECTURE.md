...
/api/v1/agent/manifest                    versao/hash do agente publicado (EPIC 22, ADR-032)
/api/v1/agent/download                    download do script do agente assinado (EPIC 22, ADR-032)
...
O roadmap detalhado (fases e EPICs) vive em `docs/development/ROADMAP.md` e `docs/development/TASKS.md` — não duplicado aqui. Resumo histórico: a base do MVP (infraestrutura, primeira API, agente, banco, dashboard, SOC Light) corresponde às EPICs 1-6; produção, governança/autenticação e hardening vieram nas EPICs 7-18; a integração RustDesk (EPIC 19, ADR-027), os relatórios/dashboard executivo (EPIC 20, ADR-029) e o polimento visual do dashboard com GSAP (EPIC 24, ADR-035) já estão concluídos; a integração com Snipe-IT feita na mesma EPIC 19 (ADR-028) foi revertida em 2026-08-11 (ADR-033) por falta de necessidade concreta de ITAM. Observabilidade de infraestrutura via Prometheus/Grafana (EPIC 21, ADR-030) foi ativada em produção em 2026-08-15 (profile `observability` em `infra/docker-compose.production.yml`; os 4 serviços — `node_exporter`, `cadvisor`, `prometheus`, `grafana` — validados na VM real e mantidos rodando continuamente por decisão operacional, apesar do alerta de memória aceito como risco, ver `docs/deployment/KNOWN_ISSUES.md`); auto-atualização do agente Windows (EPIC 22, ADR-032) foi implementada e encerrada em 2026-08-19 (updater dedicado `itcenter-agent-updater.ps1`, endpoints `/agent/manifest`/`/agent/download`, ver `docs/specs/epic-22-agent-auto-update/`); auto-detecção do ID do RustDesk (EPIC 23, ADR-034) permanece apenas planejada, sem código ainda. Depois da EPIC 24, a modernização visual do dashboard (EPIC 25) e o layout persistente de autenticação (EPIC 26) foram concluídas em 2026-08-12; a coleta do número de série da máquina pelo agente (EPIC 27) está implementada mas segue sem encerrar (falta validação em máquina virtual); e uma auditoria técnica de 2026-08-15 abriu seis EPICs de correção — EPIC 28 (segurança, praticamente concluída — falta só o CSP com nonce, ver EPIC 35), EPIC 29 (confiabilidade operacional de backup/restore, concluída) e EPIC 30 (integridade de dados do backend, concluída) já implementadas; EPIC 31 (resiliência do agente Windows) e EPIC 33 (cobertura de testes) seguem pendentes; EPIC 32 (aderência documentação-código) tem 2 dos 3 itens corrigidos. Um achado operacional adicional, fora dessa auditoria, abriu a EPIC 36 (gate de CVE do Docker Scout impraticável em `itcenter-edge-01`, 2026-08-19).
